...
// A subtle vertical wiggle is added to each vertex for a dynamic effect.
static void drawWaveform(float windowWidth, float windowHeight) {
    ...
    for (size_t i = 0; i < audioData.size(); i++) {
        ...
        float wiggleAmplitude = 2.0f;      // Adjust amplitude for subtle effect
        float wiggleFrequency = 10.0f;       // Adjust frequency for speed of wiggle
        float wiggle = std::sin(glfwGetTime() * wiggleFrequency + i * 0.1f) * wiggleAmplitude;
        yPos += wiggle;
