...
std::vector<float> samplePeriods;  // randomized period per sample (seconds)
std::vector<float> samplePhases;   // randomized phase offset (radians)
...
// JACK process callback: append incoming audio and, per sample, generate a random period and phase.
int processCallback(jack_nframes_t nframes, void* /*arg*/) {
    ...
    if (isRecording) {
        ...
        for (jack_nframes_t i = 0; i < nframes; i++) {
            // For this hacky example, choose period between 0.2 and 1.0 seconds.
...
// Instead of mapping amplitude to a full 24-bit color, we map it to one of 69 stepped colors
// along our red/orange/yellow/green gradient.
inline void amplitudeToSteppedColor(float amplitude, float& r, float& g, float& b) {
    ...
    // Determine which segment we're in (each segment spans STEPS_PER_COLOR steps)
...
// For each sample, modulate its base color over time by fading between that color and its inverse.
// f = (sin(2*pi * (time/period) + phase) + 1) / 2, then final color = (1 - f)*base + f*(1 - base).
...
// The waveform's color for each sample is computed using our stepped amplitude mapping
// and then modulated over time.
...
int main() {
    ...
    GLFWwindow* window = glfwCreateWindow(mode->width, mode->height,
        "Skeuomorphic DAW Lane (Live Stepped Colors)",
        primary, nullptr);
    if (!window) {
        glfwTerminate();
        return -1;
