...
const float PIXEL_SCALE = 1.0f;         // Global constant to increase the perceived pixel size
...
// A subtle vertical and horizontal wiggle is added to each vertex for a dynamic effect, the line width is increased,
// and translucency is applied with a marquee-like alpha effect that temporarily makes parts completely invisible.
static void drawWaveform(float windowWidth, float windowHeight) {
    ...
    // Compute a marquee base offset from time. This value will shift both the color and alpha gradients.
    float marqueeBase = std::fmod(glfwGetTime() * 0.5f, 1.0f); // adjust speed as desired

    // Increase the line width by the global pixel scale factor.
    glLineWidth(PIXEL_SCALE);
    ...
    for (size_t i = 0; i < audioData.size(); i++) {
        ...
        float rr, gg, bb;
        if (isRecording) {
            rr = gg = bb = 0.0f;  // Black while recording.
        }
        else {
        ...
        // Compute a dynamic alpha value with a marquee effect that sometimes makes the pixel completely invisible.
        float rawAlpha = std::fmod(marqueeBase + float(i) / float(audioData.size()), 1.0f);
        float sinVal = std::sin(2.0f * 3.14159265f * rawAlpha);
        float alpha = (sinVal < 0.0f) ? 0.0f : sinVal;
        ...
        // Add a subtle vertical wiggle.
        float wiggleYAmplitude = 2.0f;      // Adjust amplitude for vertical wiggle
        float wiggleYFrequency = 10.0f;       // Adjust frequency for vertical wiggle
        float wiggleY = std::sin(glfwGetTime() * wiggleYFrequency + i * 0.1f) * wiggleYAmplitude;
        yPos += wiggleY;

        // Add a subtle horizontal wiggle.
        float wiggleXAmplitude = 2.0f;      // Adjust amplitude for horizontal wiggle
        float wiggleXFrequency = 10.0f;       // Adjust frequency for horizontal wiggle
        float wiggleX = std::sin(glfwGetTime() * wiggleXFrequency + i * 0.11f) * wiggleXAmplitude;
        float xPos = bx + i * scaleX + wiggleX;
