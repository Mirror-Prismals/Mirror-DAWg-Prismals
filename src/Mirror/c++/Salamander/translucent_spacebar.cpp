...
struct Spacebar {
    ...
    float transparency = 0.4f;  // Transparency of keycap (0.0 fully transparent, 1.0 fully opaque)
};
...
// Draw the spacebar keycap in 3D with transparency
void drawSpacebarKeycap(const Spacebar& bar) {
    ...
    // TOP face
    glColor4f(baseR + 0.07f, baseG + 0.07f, baseB + 0.07f, alpha);
    glBegin(GL_QUADS);
    glVertex3f(x, y, -pressOffsetZ);
    glVertex3f(x + bw, y, -pressOffsetZ);
    ...
    // RIGHT face
    glColor4f(baseR - 0.05f, baseG - 0.05f, baseB - 0.05f, alpha);
    glBegin(GL_QUADS);
    glVertex3f(x + bw, y, -pressOffsetZ);
    glVertex3f(x + bw, y + bh, -pressOffsetZ);
    ...
    // BOTTOM face
    glColor4f(baseR - 0.02f, baseG - 0.02f, baseB - 0.02f, alpha);
    glBegin(GL_QUADS);
    glVertex3f(x, y + bh, -pressOffsetZ);
    glVertex3f(x + bw, y + bh, -pressOffsetZ);
    glVertex3f(x + bw - newDepth, y + bh - newDepth, -(pressOffsetZ + newDepth));
    ...
    // LEFT face
    glColor4f(baseR - 0.03f, baseG - 0.03f, baseB - 0.03f, alpha);
    glBegin(GL_QUADS);
    glVertex3f(x, y, -pressOffsetZ);
    glVertex3f(x, y + bh, -pressOffsetZ);
    glVertex3f(x - newDepth, y + bh - newDepth, -(pressOffsetZ + newDepth));
...
// Draw the spacebar with visible mechanism
void drawSpacebar(const Spacebar& bar) {
    // Always draw the mechanical switch first
    drawMechanicalSwitch(bar);

    // Draw the keycap with transparency if not removed
...
void keyCallback(GLFWwindow* window, int key, int scancode, int action, int mods) {
    if (key == GLFW_KEY_SPACE) {
        ...
        // Increase transparency with up arrow
        if (action == GLFW_PRESS || action == GLFW_REPEAT) {
            spacebar.transparency = std::min(1.0f, spacebar.transparency + 0.1f);
        }
    }
    else if (key == GLFW_KEY_DOWN) {
        // Decrease transparency with down arrow
        if (action == GLFW_PRESS || action == GLFW_REPEAT) {
            spacebar.transparency = std::max(0.0f, spacebar.transparency - 0.1f);
        }
    }
    else if (key == GLFW_KEY_R) {
        // Toggle keycap removal with R key
        if (action == GLFW_PRESS) {
...
int main() {
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        // Display transparency control instructions
        renderText(10.0f, 20.0f, "Use UP/DOWN to adjust transparency");
        renderText(10.0f, 40.0f, "Right-click to toggle keycap");

        // Display current transparency value
        char transparencyText[50];
        sprintf(transparencyText, "Transparency: %.1f", spacebar.transparency);
        renderText(10.0f, 60.0f, transparencyText);
