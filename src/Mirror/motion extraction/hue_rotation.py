...
# Parameters
DELAY_US = 20000  # 200,000 microseconds = 0.2 seconds
OVERLAY_OPACITY = 0.1  # Keep at 0.1 for subtle LUT effect

# --- Rainbow polarization effect ---
def rotate_hue(image, angle_deg):
    hsv = cv2.cvtColor(image, cv2.COLOR_RGB2HSV).astype(np.float32)
    hsv[..., 0] = (hsv[..., 0] + angle_deg / 2) % 180  # OpenCV hue is [0,180]
...
def main():
    with mss.mss() as sct:
        ...
        while True:
            ...
            # --- Apply rainbow polarization (auto rotating hue) ---
            output = rotate_hue(output, angle)
